...
export function Day(props: DetailProps) {
  ...
  const detail = statistics.rates.day(props.date, rates)

  const values = detail.hours.flatMap(h => h.average)
  const max = Math.min(Math.max(...[...values, 120]), 150)
    ...
    series: [
      {
        ...
        areaStyle: {
          color: {
            ...
            y2: 1,
            colorStops: [
              { offset: 0, color: 'rgba(248,113,113,5)' },
