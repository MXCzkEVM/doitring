...
export function Week(props: DetailProps) {
  ...
  const detail = statistics.sleeps.week(
    props.date,
    props.data,
  )

  const last = statistics.sleeps.week(
    dayjs.unix(props.date).subtract(1, 'w').unix(),
    props.data,
  )
          ...
          <If cond={last.data.length}>
            <Alert
              ...
              message={detail.score > last.score
                ? t('points more last week average', { score: Math.abs(detail.score - last.score) })
                : t('ponits less last week average', { score: Math.abs(detail.score - last.score) })}
            />
          </If>
